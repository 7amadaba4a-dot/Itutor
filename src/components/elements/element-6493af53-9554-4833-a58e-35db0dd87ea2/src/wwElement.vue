    ...
    <div ref="chatRoot" class="ww-chat" :class="{ 'ww-chat--disabled': isDisabled }" :style="containerStyles">
        ...
        <div ref="messagesContainer" class="ww-chat__messages" :style="messagesContainerStyles">
            <MessageList
                ...
                :message-status-display="messageStatusDisplay"
                :sent-icon="sentIcon"
                :sent-icon-color="sentIconColor"
                :delivered-icon="deliveredIcon"
                :delivered-icon-color="deliveredIconColor"
                :read-icon="readIcon"
                :read-icon-color="readIconColor"
                :message-status-size="messageStatusSize"
                :message-status-spacing="messageStatusSpacing"
    ...
    setup(props, { emit }) {
        ...
        const messages = computed(() => {
            ...
            return rawMessages.value.map(message => {
                ...
                // Map message status
                const messageStatus = resolveMapping(message, props.content?.mappingMessageStatus, 'status') || 'sent';
        ...
        const messageStatusDisplay = computed(() => props.content?.messageStatusDisplay !== false);
        const sentIcon = computed(() => props.content?.sentIcon || 'check');
        const sentIconColor = computed(() => props.content?.sentIconColor || '#64748b');
        const deliveredIcon = computed(() => props.content?.deliveredIcon || 'check-check');
        const deliveredIconColor = computed(() => props.content?.deliveredIconColor || '#64748b');
        const readIcon = computed(() => props.content?.readIcon || 'check-check');
        const readIconColor = computed(() => props.content?.readIconColor || '#10b981');
        const messageStatusSize = computed(() => props.content?.messageStatusSize || '14px');
        const messageStatusSpacing = computed(() => props.content?.messageStatusSpacing || '4px');
        ...
        - \`messageStatus\`: Message status (sent/delivered/read)
...
.ww-chat {
    ...
    /* Message status styles */
    --ww-chat-status-size: v-bind('messageStatusSize');
    --ww-chat-status-spacing: v-bind('messageStatusSpacing');
